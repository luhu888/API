...
def gennerator():
    number = codelist[random.randint(0, len(codelist))]['code']  # 地区项
    number += str(random.randint(1930, 2013))  # 年份项
    da = date.today() + timedelta(days=random.randint(1, 366))  # 月份和日期项
    number += da.strftime('%m%d')
    number += str(random.randint(100, 300))  # 顺序号简单处理
    i = 0
    count = 0
    ...
    for i in range(0, len(number)):
        count += int(number[i]) * weight[i]
    number = number + checkcode[str(count % 11)]  # 算出校验码
    # print number
    return number
